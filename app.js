...
io.on('connection', function(socket) {
    ...
    socket.on('new user', function(data, callback) { //espera por um evento de novo utilizador
        ...
        if (array.indexOf(data.toLowerCase()) != -1) {
            io.emit('user error', 'Esse utilizador ja foi usado, tente novamente');
            callback(false);
            ...
            io.emit('user error', 'A sala está cheia, tente mais tarde');
        }
    });
